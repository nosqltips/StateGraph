...
use stategraph_storage::{Storage, StorageError};
...
impl Repository {
    ...
    pub fn merge(
        ...
        let result = stategraph_core::merge::three_way_merge(
            &resolver,
    ...
    fn store_object_tree(&self, obj: &Object) -> Result<(), RepoError> {
        ...
        if let Object::Node(node) = obj {
            let children = match node {
                stategraph_core::Node::Map(entries) => entries.values().copied().collect::<Vec<_>>(),
...
mod tests {
    ...
    fn test_diff_value_change() {
        ...
        assert_eq!(ops.len(), 1);
        match &ops[0] {
            stategraph_core::DiffOp::SetValue { path, .. } => {
