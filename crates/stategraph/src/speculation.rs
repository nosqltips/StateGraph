...
use stategraph_core::{DiffOp, Object, ObjectId, ObjectResolver};
use stategraph_storage::ObjectStore;
...
pub enum SpecError {
    ...
    Storage(#[from] stategraph_storage::StorageError),
}
...
impl SpeculationManager {
    ...
    pub fn compare(
        ...
        for &handle in handles {
            ...
            let diff_ops =
                stategraph_core::diff::diff(&resolver, &spec.base_root, &spec.current_root);
...
impl<'a> ObjectStore for OverlayObjectStore<'a> {
    fn get_object(&self, id: &ObjectId) -> Result<Option<Object>, stategraph_storage::StorageError> {
        Ok(self.0.resolve_to_object(id))
    }

    fn put_object(&self, obj: &Object) -> Result<ObjectId, stategraph_storage::StorageError> {
        // Delegate to the underlying store
        self.0.store.put_object(obj)
    }

    fn has_object(&self, id: &ObjectId) -> Result<bool, stategraph_storage::StorageError> {
...
mod tests {
    use super::*;
    use stategraph_core::IntentCategory;
    ...
    fn test_speculate_modify_and_read() {
        ...
        let original = tree::tree_get(
            &store,
            &root_id,
            &stategraph_core::StatePath::parse("/storage/type").unwrap(),
        )
        .unwrap();
    ...
    fn test_delete_in_speculation() {
        ...
        let original = tree::tree_get(
            &store,
            &root_id,
            &stategraph_core::StatePath::parse("/network").unwrap(),
        );
